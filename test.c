...
mlx_keyfunc key_hook(mlx_key_data_t key, void *param)
{
    ...
    if(key.key == MLX_KEY_S)
    {
        ...
        {
            x += cosf((playerangel+90) * M_PI / 180.0) * 0.15;
            y -= sinf((playerangel+90) * M_PI / 180.0) * 0.15;
        }
    ...
    if(key.key == MLX_KEY_D)
    {
        ...
        if (distance1 > 0.2 && distance2 > 0.2)
        {
        x -= cosf((playerangel) * M_PI / 180.0) * 0.15;
        y += sinf((playerangel) * M_PI / 180.0) * 0.15;
        }
    }
...
int my_mlx_loop_hook(void *param)
{
    ...
    {
        double a = angle;
        double distance = hits(a) * cosf((a - playerangel) * M_PI / 180.0);

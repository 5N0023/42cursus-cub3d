...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/04 20:15:13 by mlektaib          #+#    #+#             */
/*   Updated: 2023/09/04 20:15:14 by mlektaib         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_reload_animation(t_data *data)
{
	...
	while (i < 46)
	{
		...
		if (!texture)
			exit(1);
		data->player.gun.normal.gunreload[i] = mlx_texture_to_image(data->mlx,
				texture);
...
void	load_shoot_animation(t_data *data)
{
	...
	while (i < 10)
	{
		...
		if (!texture)
			exit(1);
		data->player.gun.normal.gunshoot[i] = mlx_texture_to_image(data->mlx,
				texture);
...
void	load_bullets_count(t_data *data)
{
	...
	while (i < 25)
	{
		...
		if (!texture)
			exit(1);
		data->player.gun.normal.bullets[i] = mlx_texture_to_image(data->mlx,
				texture);
